...
const getJobSchema = (t: any) =>
  z.object({
    ...
    clientId: z.number({
      message: t("mobile.jobs.modal.newJob.form.clientRequired"),
      error: t("mobile.jobs.modal.newJob.form.clientValid"),
    }),
    type: z.enum(
      ["repair", "installation", "maintenance", "quote", "emergency"],
      {
        message: t("mobile.jobs.modal.newJob.form.typeRequired"),
      }
    ),
    startDate: z
      .string()
    ...
    duration: z
      .number({
        message: t("mobile.jobs.modal.newJob.form.durationRequired"),
        error: t("mobile.jobs.modal.newJob.form.durationNumber"),
      })
      .min(0.5, t("mobile.jobs.modal.newJob.form.durationMin")),
    hourlyRate: z
      .number({
        message: t("mobile.jobs.modal.newJob.form.hourlyRateRequired"),
        error: t("mobile.jobs.modal.newJob.form.hourlyRateNumber"),
      })
      .min(0, t("mobile.jobs.modal.newJob.form.hourlyRateMin")),
    materialsCost: z
      .number({
        error: t("mobile.jobs.modal.newJob.form.materialsCostNumber"),
      })
      .default(0)
      .optional(),
  ...
  const handleFormSubmit = async (values: JobFormValues) => {
    try {
      ...
      toast({
        title: "Errore",
        description:
          "Si è verificato un errore durante la creazione del lavoro",
        variant: "destructive",
      });
    }
  };
          ...
          >
                  ...
                  >
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue
                          placeholder={t(
                            "mobile.jobs.modal.newJob.form.clientPlaceholder"
                          )}
                        />
                      </SelectTrigger>
                    </FormControl>
                  ...
                  <Select value={field.value} onValueChange={field.onChange}>
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue
                          placeholder={t(
                            "mobile.jobs.modal.newJob.form.typePlaceholder"
                          )}
                        />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      <SelectItem value="repair">
                        {t("mobile.jobs.type.repair")}
                      </SelectItem>
                      <SelectItem value="installation">
                        {t("mobile.jobs.type.installation")}
                      </SelectItem>
                      <SelectItem value="maintenance">
                        {t("mobile.jobs.type.maintenance")}
                      </SelectItem>
                      <SelectItem value="quote">
                        {t("mobile.jobs.type.quote")}
                      </SelectItem>
                      <SelectItem value="emergency">
                        {t("mobile.jobs.type.emergency")}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <FormMessage />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    {t("mobile.jobs.modal.newJob.form.startDate")}
                  </FormLabel>
                  <FormControl>
                ...
                {photos.map((photo, index) => (
                  <div key={index} className="relative w-20 h-20">
                    <img
                      src={photo}
                      alt={`${t(
                        "mobile.jobs.modal.newJob.form.photoPreview"
                      )} ${index + 1}`}
