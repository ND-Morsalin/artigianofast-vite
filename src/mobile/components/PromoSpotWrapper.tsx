...
export default function PromoSpotWrapper() {
    ...
    const fetchSpots = async () => {
      try {
        ...
        if (!data || !Array.isArray(data) || data.length === 0) {
          console.log(
            "Nessuno spot ricevuto dall'API, aggiungo spot demo client-side"
          );
          data = [demoSpot];
        } else {
  ...
  useEffect(() => {
    ...
    spots.forEach((spot) => {
      ...
      if (spot.displayDuration && spot.displayInterval) {
        console.log(
          `Configuro intermittenza per spot ${spot.id}: ${spot.displayDuration}s ogni ${spot.displayInterval}s`
        );
  ...
  const filterSpotsByPosition = (position: string) => {
    ...
    console.log(
      `Filtraggio spot per posizione "${position}". Spot disponibili:`,
      spots
    );
    ...
    const filtered = spots.filter((spot) => {
      ...
      console.log(
        `Spot ${spot.id} - posizione ${spot.position} corrisponde a "${position}"? ${positionMatch}`
      );
      ...
      console.log(
        `Spot ${spot.id} - status: ${spot.status}, attivo? ${isActive}`
      );
      ...
      console.log(
        `Spot ${spot.id} - date: ora=${now}, inizio=${startDate}, fine=${endDate}, in range? ${isInDateRange}`
      );
      ...
      if (spot.timeRanges && spot.timeRanges.length > 0) {
        ...
        isInTimeRange = spot.timeRanges.some(
          (range: { startTime?: string; endTime?: string }) => {
            if (!range || !range.startTime || !range.endTime) return false;

            const [startHour, startMinute] = range.startTime
              .split(":")
              .map(Number);
        ...
        console.log(
          `Spot ${
            spot.id
          } - ora corrente: ${now.getHours()}:${now.getMinutes()}, in range orario? ${isInTimeRange}`
        );
      ...
      if (spot.visiblePages) {
        ...
        // Se è una stringa che contiene una virgola, potrebbe essere un elenco separato da virgole
          ...
          typeof spot.visiblePages === "string" &&
          spot.visiblePages.includes(",")
        ) {
          const pages = spot.visiblePages
            .split(",")
            .map((p: string) => p.trim());
          isOnVisiblePage = pages.some((page: string) =>
            location.startsWith(page)
          );
          console.log(
            `Spot ${spot.id} - pagina corrente: ${location}, pagine visibili: ${pages}, visibile? ${isOnVisiblePage}`
          );
        }
        // Se è un array
          ...
          if (spot.visiblePages.length === 0) {
            isOnVisiblePage = true;
            console.log(
              `Spot ${spot.id} - array vuoto, visibile su tutte le pagine`
            );
          } else {
            ...
            console.log(
              `Spot ${spot.id} - pagina corrente: ${location}, pagine visibili: ${spot.visiblePages}, visibile? ${isOnVisiblePage}`
            );
          }
        }
        // Singola stringa
          ...
          console.log(
            `Spot ${spot.id} - pagina corrente: ${location}, pagina visibile: ${spot.visiblePages}, visibile? ${isOnVisiblePage}`
          );
        }
        // Se non è nessuno dei formati sopra, considera visibile ovunque (per sicurezza)
          ...
          isOnVisiblePage = true;
          console.log(
            `Spot ${spot.id} - formato visiblePages non riconosciuto, mostrato per sicurezza:`,
            spot.visiblePages
          );
        }
      ...
      console.log(
        `Spot ${spot.id} - TUTTE LE CONDIZIONI SODDISFATTE, mostro lo spot`
      );
      return true;
    });
  ...
  const createSpot = (spot: any, styleClass: string) => {
    ...
    return (
      <div
        className={`bg-white rounded-md overflow-hidden shadow-lg ${styleClass}`}
      >
        <button
        ...
        <div
          ...
          style={{
            cursor:
              spot.enableRedirect && spot.redirectUrl ? "pointer" : "default",
          }}
        >
          {spot.images && spot.images.length > 0 ? (
            <div className="relative w-full h-full">
              <img
                ...
                onError={(e) => {
                  console.error(
                    `Errore caricamento immagine: ${spot.images[0]}`
                  );
                  e.currentTarget.src =
                    "https://via.placeholder.com/300x100?text=Errore+Immagine";
                }}
              />
              <div className="absolute bottom-0 left-0 right-0 bg-black/60 p-2">
                <h3 className="font-bold text-sm text-white">{spot.title}</h3>
                {spot.content && (
                  <div className="text-xs text-white/80">{spot.content}</div>
                )}
              </div>
            </div>
          ) : (
            <div className="p-3 flex-1 flex flex-col justify-center">
              ...
              {spot.content && (
                <div className="text-xs text-gray-700">{spot.content}</div>
              )}
            </div>
          )}
        </div>
  ...
  return (
    <>
      ...
      {headerSpot &&
        createSpot(
          headerSpot,
          getSpotStyle(headerSpot, "fixed top-0 left-0 right-0 z-50")
        )}
      ...
      {footerSpot &&
        createSpot(
          footerSpot,
          getSpotStyle(footerSpot, "fixed bottom-16 left-0 right-0 z-50")
        )}
      ...
      {sidebarSpot &&
        createSpot(
          sidebarSpot,
          getSpotStyle(sidebarSpot, "fixed top-1/4 left-0 z-50")
        )}
      ...
      {popupSpot &&
        createSpot(
          ...
          getSpotStyle(
            popupSpot,
            "fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 shadow-xl"
          )
        )}
    </>
  );
