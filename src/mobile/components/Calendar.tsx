...
export default function MobileCalendar() {
  ...
  const getFormattedDateDisplay = () => {
    if (viewMode === "day") {
      ...
      const weekStart = startOfWeek(selectedDate, { weekStartsOn: 1 });
      const weekEnd = endOfWeek(selectedDate, { weekStartsOn: 1 });
      return `${format(weekStart, "d")} - ${format(weekEnd, "d")} ${format(
        weekEnd,
        "MMMM yyyy",
        { locale: it }
      )}`;
                    ...
                    return (
                      ...
                      >
                        <div className="p-3 flex justify-between items-center bg-gray-50 border-b">
                          ...
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-600/10 text-primary`}
                          >
                        ...
                        <div className="p-3">
                          <div
                            className={`pl-3 border-l-4 ${getJobColor(
                              job.type
                            )}`}
                          >
              ...
              {viewMode === "week" && (
                <div className="grid grid-cols-7 gap-1">
                  {Array.from({ length: 7 }).map((_, i) => {
                    ...
                    const dayJobs = jobsWithClients.filter(
                      (job) =>
                        format(new Date(job.startDate), "yyyy-MM-dd") ===
                            ...
                            dayJobs
                              ...
                              .map((job) => {
                                ...
                                return (
                                  <div
                                    key={job.id}
                                    className={`text-xs p-1 mb-1 rounded border-l-2 ${getJobColor(
                                      job.type
                                    )} bg-gray-50`}
                                  >
