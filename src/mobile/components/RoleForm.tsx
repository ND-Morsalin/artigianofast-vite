...
const getAvailablePermissions = (t: any) => [
  {
    ...
    options: [
      {
        id: "jobs.create",
        label: t("mobile.forms.role.permissionsList.create"),
      },
  ...
  },
  {
    ...
    options: [
      ...
      {
        id: "clients.edit",
        label: t("mobile.forms.role.permissionsList.edit"),
      },
      {
        id: "clients.view",
        label: t("mobile.forms.role.permissionsList.view"),
      },
    ],
  },
  {
    ...
    options: [
      {
        ...
        label: t("mobile.forms.role.permissionsList.assign"),
      },
      {
        id: "assignments.approve",
        label: t("mobile.forms.role.permissionsList.approve"),
      },
    ],
  },
];
...
export default function RoleForm() {
        ...
        try {
          ...
          if (data) {
            ...
            const permissions = availablePermissions.reduce((acc, category) => {
              category.options.forEach((option) => {
                const value =
                  permissionsObj[option.id] === true ? "true" : "false";
                acc[option.id] = value;
              });
              return acc;
            }, {} as Record<string, string>);
  ...
  const createMutation = useMutation({
    mutationFn: async (values: RoleFormValues) => {
      ...
      const normalizedPermissions = Object.fromEntries(
        Object.entries(values.permissions || {}).map(([k, v]) => [
          k,
          v === "true",
        ])
      );
  ...
  return (
    <MobileLayout
      title={
        isEditMode
          ? t("mobile.forms.role.editTitle")
          : t("mobile.forms.role.title")
      }
      showBackButton={true}
      rightAction={
        <Button
          variant="ghost"
          size="icon"
          onClick={() => setLocation("/mobile/settings/roles")}
        >
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder={t("mobile.forms.role.namePlaceholder")}
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        placeholder={t(
                          "mobile.forms.role.descriptionPlaceholder"
                        )}
                        {...field}
                        value={field.value || ""}
                      />
                  ...
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                    <div className="space-y-0.5">
                      <FormLabel className="text-base">
                        {t("mobile.forms.role.administrator")}
                      </FormLabel>
                      <FormDescription>
              ...
              <Button type="submit" className="w-full" disabled={isSaving}>
                ...
                {isEditMode
                  ? t("mobile.forms.role.updateButton")
                  : t("mobile.forms.role.createButton")}
              </Button>
            </form>
          </Form>
        )}
