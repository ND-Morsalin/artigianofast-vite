...
interface PromotionalSpot {
  ...
  imageDisplayType?: "slideshow" | "single" | string;
  ...
  content?: string;
  height?: number;
  textAnimationType?: "scroll" | "static" | string;
}
...
export default function PromoSpot({ position }: PromoSpotProps) {
  ...
  // Browser setTimeout returns a number, so use Record<number, number> (or ReturnType<typeof setTimeout>)
  const [spotTimers, setSpotTimers] = useState<Record<string, number>>({});
  ...
  const filteredSpots =
    spots?.filter((spot) => {
      ...
      if (spot.visiblePages === "all") {
        console.log(
          `Spot ${spot.id} - visiblePages = 'all', valido per tutte le pagine`
        );
        return true;
      }
      ...
      const pageMatch =
        Array.isArray(spot.visiblePages) &&
        spot.visiblePages.some((page: string) => location.startsWith(page));
      console.log(
        `Spot ${spot.id} - Verifica pagina corrente ${location} in visiblePages: ${pageMatch}`
      );
      return pageMatch;
    }) || [];
  ...
  useEffect(() => {
    ...
    if (position === "popup" && currentSpot?.displayDuration) {
      ...
      // Salva il timer
      setSpotTimers((prev) => ({ ...prev, [String(currentSpot.id)]: timer }));
    ...
    if (
      currentSpot?.imageDisplayType === "slideshow" &&
      currentSpot.images &&
  ...
  useEffect(() => {
    if (
      !currentSpot ||
      !currentSpot.displayInterval ||
      currentSpot.displayInterval <= 0
    ) {
    ...
    console.log(
      `Configurazione spot intermittente - ID: ${currentSpot.id}, Durata: ${displayDuration}s, Intervallo: ${intervalDuration}s`
    );

    // Timer per nascondere lo spot dopo la durata di visualizzazione
    const hideTimer = setTimeout(() => {
      console.log(
        `Nascondo spot ${currentSpot.id} dopo ${displayDuration} secondi`
      );
      setIsVisible(false);
  ...
  const handleClose = () => {
    ...
    if (position === "popup" && currentSpot) {
      const timerId = spotTimers[String(currentSpot.id)];
  ...
  const imagesArray =
    currentSpot.images && Array.isArray(currentSpot.images)
      ? currentSpot.images
      : typeof currentSpot.images === "string"
      ? currentSpot.images.split(",").map((img: string) => img.trim())
      : [];
    ...
    >
      ...
      <div
        className={`flex h-full w-full ${
          currentSpot.enableRedirect ? "cursor-pointer" : "cursor-default"
        }`}
          ...
          >
            <img
              ...
              onError={(e) => {
                ...
                e.currentTarget.src =
                  "https://via.placeholder.com/300x100?text=Immagine+non+disponibile";
              }}
            />
          </div>
        )}
