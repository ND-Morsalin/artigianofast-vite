...
export const MobilePieChart: React.FC<{ data: ChartData[]; title: string }> = ({
  data,
  title,
}) => {
          ...
          >
            {data.map((entry, index) => (
              <Cell
                ...
                fill={MOBILE_COLORS[index % MOBILE_COLORS.length]}
              />
            ))}
          </Pie>
          <Tooltip />
...
export const MobileBarChart: React.FC<{ data: ChartData[]; title: string }> = ({
  data,
  title,
}) => {
  const { t } = useTranslation();
...
export const MobileLineChart: React.FC<{
  data: ChartData[];
  title: string;
}> = ({ data, title }) => {
  const { t } = useTranslation();
...
export const MobileAreaChart: React.FC<{
  data: ChartData[];
  title: string;
}> = ({ data, title }) => {
  const { t } = useTranslation();
...
export const MobileRadarChart: React.FC<{
  data: ChartData[];
  title: string;
}> = ({ data, title }) => {
  const { t } = useTranslation();
...
export const MobileCharts: React.FC<MobileChartsProps> = ({
  ...
  const chartTabs = [
    ...
    {
      id: "performance",
      title: t("analytics.tabs.performance", "Performance"),
    },
  ];
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        ...
        {activeChart === "performance" && (
          <MobileRadarChart
            ...
            title={t(
              "analytics.charts.featureAdoption",
