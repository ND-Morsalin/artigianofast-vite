...
const getRegistrationSchema = (t: any) =>
  z.object({
    jobId: z.number({
      error: t("mobile.jobs.modal.jobRegistration.form.jobValid"),
    }),
    activityId: z.number({
      error: t("mobile.jobs.modal.jobRegistration.form.activityValid"),
    }),
    status: z.enum(["scheduled", "in_progress", "completed", "cancelled"], {}),
    duration: z
      .number({
        error: t("mobile.jobs.modal.jobRegistration.form.durationNumber"),
      })
      .min(0.5, t("mobile.jobs.modal.jobRegistration.form.durationMin")),
    startDate: z
      .string()
      .min(1, t("mobile.jobs.modal.jobRegistration.form.startDateRequired")),
...
export function JobRegistrationDialog({
  ...
  // const canViewRegistrationActivity = hasPermission(
  //   "canViewRegistrationActivity"
  // );
  // const canViewRegistrationDuration = hasPermission(
  //   "canViewRegistrationDuration"
  // );
  const canViewRegistrationPhotos = hasPermission("canViewRegistrationPhotos");
  // const canViewRegistrationLocation = hasPermission(
  //   "canViewRegistrationLocation"
  // );
  ...
  const canViewRegistrationMaterials = hasPermission(
    "canViewRegistrationMaterials"
  );
  // const canViewRegistrationSignature = hasPermission(
  //   "canViewRegistrationSignature"
  // );
  ...
  const jobs = allJobs.filter((job: any) =>
    filteredJobStatus.includes(job.status)
  );
  ...
  const activities = useMemo(() => {
    ...
    const filteredActivities = allActivities.filter(
      (activity: any) => activity.jobTypeId === jobTypeId
    );
  ...
  const registerActivity = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: t(
          "mobile.jobs.modal.jobRegistration.activityRegisteredDescription"
        ),
      });
    ...
    <Dialog open={isOpen} onOpenChange={handleDialogClose}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader className="flex justify-between items-start">
          <div>
            <DialogTitle>
              {t("mobile.jobs.modal.jobRegistration.registerActivityTitle")}
            </DialogTitle>
            {job && job.title && (
              <div className="text-sm text-gray-600 mt-1">
                <p>{job.title}</p>
                <p className="text-xs">
                  {t("mobile.jobs.modal.jobRegistration.jobStartDate", {
                    date: job.startDate
                      ? format(new Date(job.startDate), "dd MMM yyyy, HH:mm", {
                          locale: it,
                        })
                      : "",
                  })}
            ...
            >
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="mb-2">
                      <FormLabel>
                        {t("mobile.jobs.modal.jobRegistration.form.jobLabel")}
                      </FormLabel>
                      <Select
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue
                              placeholder={t(
                                "mobile.jobs.modal.jobRegistration.form.jobPlaceholder"
                              )}
                            />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          <SelectItem value="0">
                            {t(
                              "mobile.jobs.modal.jobRegistration.form.jobSelect"
                            )}
                          </SelectItem>
                          {jobs.map((job: any) => (
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="mb-2">
                      <FormLabel>
                        {t(
                          "mobile.jobs.modal.jobRegistration.form.activityLabel"
                        )}
                      </FormLabel>
                      <Select
                        value={field.value.toString()}
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue
                              placeholder={t(
                                "mobile.jobs.modal.jobRegistration.form.activityPlaceholder"
                              )}
                            />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          <SelectItem value="0">
                            {t(
                              "mobile.jobs.modal.jobRegistration.form.activitySelect"
                            )}
                          </SelectItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="mb-2">
                    <FormLabel>
                      {t("mobile.jobs.modal.jobRegistration.form.statusLabel")}
                    </FormLabel>
                    <Select value={field.value} onValueChange={field.onChange}>
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue
                            placeholder={t(
                              "mobile.jobs.modal.jobRegistration.form.statusPlaceholder"
                            )}
                          />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        <SelectItem value="scheduled">
                          {t(
                            "mobile.jobs.modal.jobRegistration.form.statusScheduled"
                          )}
                        </SelectItem>
                        <SelectItem value="in_progress">
                          {t(
                            "mobile.jobs.modal.jobRegistration.form.statusInProgress"
                          )}
                        </SelectItem>
                        <SelectItem value="completed">
                          {t(
                            "mobile.jobs.modal.jobRegistration.form.statusCompleted"
                          )}
                        </SelectItem>
                        <SelectItem value="cancelled">
                          {t(
                            "mobile.jobs.modal.jobRegistration.form.statusCancelled"
                          )}
                        </SelectItem>
                      </SelectContent>
                    </Select>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="mb-2">
                    <FormLabel>
                      {t(
                        "mobile.jobs.modal.jobRegistration.form.startDateLabel"
                      )}
                    </FormLabel>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="mb-2">
                    <FormLabel>
                      {t(
                        "mobile.jobs.modal.jobRegistration.form.durationLabel"
                      )}
                    </FormLabel>
                    <FormControl>
                      <Input
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="mb-2">
                      <FormLabel>
                        {t(
                          "mobile.jobs.modal.jobRegistration.form.materialsLabel"
                        )}
                      </FormLabel>
                      <FormControl>
                        <Textarea
                          placeholder={t(
                            "mobile.jobs.modal.jobRegistration.form.materialsPlaceholder"
                          )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="mb-2">
                      <FormLabel>
                        {t(
                          "mobile.jobs.modal.jobRegistration.form.materialsCostLabel"
                        )}
                      </FormLabel>
                      <FormControl>
                        <Input
              ...
              {canViewRegistrationNotes && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="mb-2">
                      <FormLabel>
                        {t("mobile.jobs.modal.jobRegistration.form.notesLabel")}
                      </FormLabel>
                      <FormControl>
                        <Textarea
                          placeholder={t(
                            "mobile.jobs.modal.jobRegistration.form.notesPlaceholder"
                          )}
                          className="min-h-[60px]"
                          {...field}
                        />
              ...
              {canViewRegistrationPhotos && (
                <div className="space-y-1 mb-2">
                  <FormLabel>
                    {t("mobile.jobs.modal.jobRegistration.form.photosLabel")}
                  </FormLabel>
                  <div className="flex items-center space-x-2">
                    <label className="flex-1">
                      <div className="cursor-pointer px-3 py-2 border border-gray-300 rounded-lg text-sm flex items-center hover:bg-gray-50">
                        <span className="mr-2">
                          {t(
                            "mobile.jobs.modal.jobRegistration.form.chooseFile"
                          )}
                        </span>
                        {photos.length > 0 ? (
                          <span className="text-gray-500">
                            {photos.length}{" "}
                            {t(
                              "mobile.jobs.modal.jobRegistration.form.selectedFiles"
                            )}
                          </span>
                        ) : (
                          <span className="text-gray-500">
                            {t(
                              "mobile.jobs.modal.jobRegistration.form.noFilesSelected"
                            )}
                          </span>
                        )}
                      </div>
                        ...
                        >
                          {mediaFiles[index]?.type.startsWith("video/") ? (
                            ...
                            >
                              ...
                              <p>
                                {t(
                                  "mobile.jobs.modal.jobRegistration.form.videoNotSupported"
                                )}
                              </p>
                            </video>
                          ) : (
                          ...
                          <button
                            ...
                            aria-label={t(
                              "mobile.jobs.modal.jobRegistration.form.removeFile"
                            )}
                          >
                            ×
                          </button>
              ...
              <div className="grid grid-cols-2 gap-2 pt-4">
                ...
                >
                  {registerActivity.isPending
                    ? t("mobile.jobs.modal.jobRegistration.registering")
                    : t(
                        "mobile.jobs.modal.jobRegistration.registerActivityButton"
                      )}
                </Button>
              </div>
            </form>
