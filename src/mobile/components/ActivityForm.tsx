...
export default function ActivityForm() {
  ...
  const processJobTypesIds = (
    jobTypesIdsString: string | undefined | null
  ): number[] => {
    ...
    try {
      ...
      if (
        jobTypesIdsString.startsWith("[") &&
        jobTypesIdsString.endsWith("]")
      ) {
        return JSON.parse(jobTypesIdsString);
      } else {
        return jobTypesIdsString
          .split(",")
          .map((id) => id.trim())
          .filter((id) => id !== "")
  ...
  useEffect(() => {
    if (activityData) {
      ...
      form.reset({
        ...
        defaultDuration: activityData.defaultDuration
          ? activityData.defaultDuration.toString()
          : "",
        defaultRate: activityData.defaultRate
          ? activityData.defaultRate.toString()
          : "",
      ...
      try {
        ...
        const dataToSend = {
          ...
          defaultDuration: values.defaultDuration
            ? parseFloat(values.defaultDuration)
            : undefined,
          defaultRate: values.defaultRate
            ? parseFloat(values.defaultRate)
            : undefined,
        };
  ...
  const updateActivity = useMutation({
    mutationFn: async (values: ActivityFormValues) => {
      ...
      try {
        ...
        console.log(
          "Dati formattati da inviare per l'aggiornamento:",
          JSON.stringify(dataToSend, null, 2)
        );
        ...
        console.log(
          "Risposta del server per aggiornamento:",
          response.status,
          response.statusText
        );
    ...
    onError: (error: Error) => {
      ...
      toast({
        ...
        description:
          error.message ||
          "Si è verificato un errore durante l'aggiornamento dell'attività",
        variant: "destructive",
      });
    },
  });
