...
export default function CollaboratorsSettings() {
  ...
  const filteredCollaborators = collaborators.filter(
    (collaborator) =>
      ...
      (collaborator.phone &&
        collaborator.phone.toLowerCase().includes(searchQuery.toLowerCase()))
  );
  ...
  const handleDeleteClick = (id: number, name: string) => {
    if (
      window.confirm(
        `Sei sicuro di voler eliminare il collaboratore "${name}"?`
      )
    ) {
  ...
  const atLimit =
    maxCollaborators !== -1 && collaborators.length >= maxCollaborators;
  const rightAction = (
    <Button
      size="sm"
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center space-x-2">
                    <Button
                      ...
                      onClick={() =>
                        setLocation(
                          `/mobile/settings/collaborators/${collaborator.id}`
                        )
                      }
                    >
                    ...
                    <Button
                      ...
                      size="icon"
                      onClick={() =>
                        handleDeleteClick(collaborator.id, collaborator.name)
                      }
                    >
          ...
          <div className="py-8 text-center">
            ...
            {atLimit && (
              <div className="mt-2 text-xs text-amber-700">
                Hai raggiunto il limite del tuo piano.
              </div>
            )}
          </div>
        )}
  ...
  return (
    <FeatureGate
      feature="collaborator_management"
      fallback={
        <MobileLayout title="Collaboratori">
