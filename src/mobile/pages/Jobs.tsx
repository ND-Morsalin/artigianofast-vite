...
const getJobTypes = (t: any) => [
  { value: "repair", label: t("mobile.jobs.form.type.repair") },
  { value: "installation", label: t("mobile.jobs.form.type.installation") },
  { value: "maintenance", label: t("mobile.jobs.form.type.maintenance") },
  { value: "quote", label: t("mobile.jobs.form.type.quote") },
  { value: "emergency", label: t("mobile.jobs.form.type.emergency") },
];

const getJobStatuses = (t: any) => [
  { value: "scheduled", label: t("mobile.jobs.status.scheduled") },
  { value: "in_progress", label: t("mobile.jobs.status.in_progress") },
  { value: "completed", label: t("mobile.jobs.status.completed") },
  { value: "cancelled", label: t("mobile.jobs.status.cancelled") },
];

// Opzioni di ordinamento - verranno tradotte dinamicamente
const getSortOptions = (t: any) => [
  { value: "date_desc", label: t("mobile.jobs.sort.dateDesc") },
  { value: "date_asc", label: t("mobile.jobs.sort.dateAsc") },
  { value: "name_asc", label: t("mobile.jobs.sort.nameAsc") },
  { value: "name_desc", label: t("mobile.jobs.sort.nameDesc") },
  { value: "status", label: t("mobile.jobs.sort.status") },
];
...
export default function Jobs(props: any) {
  // Estraiamo isRegistrationPage dalle props (se presente)
  const isRegistrationPage = props.isRegistrationPage || false;
  ...
  const [selectedFilters, setSelectedFilters] = useState<{
    ...
    types: [],
    statuses: isRegistrationPage ? ["scheduled", "in_progress"] : ["scheduled"],
  });
  ...
  const enrichedJobs = jobs.map((job) => ({
    ...job,
    client: (clients as any[]).find((c) => c.id === job.clientId) || {
  ...
  const filteredJobs = enrichedJobs.filter((job) => {
    // Se è la pagina di registrazione, escludiamo i lavori completati e cancellati
    if (
      isRegistrationPage &&
      ...
      >
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center">
            {/* Mostra pulsante registra attività solo nella pagina di registrazione */}
            {isRegistrationPage && (
              <Button
                variant="ghost"
                size="icon"
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                >
                  {t("mobile.jobs.actions.viewDetails")}
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  onClick={() => {
                    // Apri il popup di modifica lavoro con i dati precaricati
                    const event = new CustomEvent("openModal", {
                      detail: {
                        modalId: "new-job-modal",
                        jobId: job.id,
                      },
                    });
                    ...
                    if (confirm(t("mobile.jobs.actions.confirmCancel"))) {
                      ...
                      mobileApiCall("PATCH", `${BASE_URL}/api/jobs/${job.id}`, {
                        ...
                        .then(() => {
                          ...
                          toast({
                            title: t("mobile.jobs.actions.cancelSuccess"),
                            description: t(
                              "mobile.jobs.actions.cancelSuccessDesc"
                            ),
                          });
                ...
                >
                  ...
                  <span>{t("mobile.jobs.actions.cancel")}</span>
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex items-center justify-between mt-3">
            <div
              className={`px-2 py-1 rounded-full text-xs ${jobTypeColor} text-white`}
            >
  ...
  // Handler per l'apertura del modale di nuovo lavoro o registrazione attività
  const handleAddNewJob = () => {
    if (isRegistrationPage) {
      // Se siamo nella pagina registrazioni, apri il popup di registrazione attività
      setSelectedJobId(undefined);
      setIsJobRegistrationOpen(true);
    } else {
      // Altrimenti apri il popup per creare un nuovo lavoro
      const event = new CustomEvent("openModal", {
        detail: {
          modalId: "new-job-modal",
          jobId: null, // Nuovo lavoro, nessun jobId
        },
      });
      document.dispatchEvent(event);
    }
  ...
  // Determina il titolo in base alla prop isRegistrationPage e al percorso corrente
  const pageTitle =
    isRegistrationPage || location === "/mobile/registration"
      ? "Registrazioni"
      : "Lavori";
  ...
  return (
    ...
    >
      <div className="p-4 pb-20">
                      ...
                      {jobTypes.map((type) => (
                        ...
                        >
                          <Checkbox
                            ...
                            onCheckedChange={() =>
                              handleTypeFilterChange(type.value)
                            }
                          />
        ...
        <div>
          {isJobsLoading ? (
            ...
            <div className="py-8 text-center">
              ...
              <Button className="mt-4" onClick={handleAddNewJob}>
                {isRegistrationPage ? (
                  <>
                    <CalendarPlus size={16} className="mr-2" />
                    Pianifica nuovo lavoro
                  </>
                ) : (
                  <>
        ...
        {/* Dialog per visualizzare i dettagli del lavoro */}
        <JobDetailsDialog
