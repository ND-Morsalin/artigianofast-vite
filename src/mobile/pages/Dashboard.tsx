...
export default function MobileDashboard() {
  ...
  const [greeting, setGreeting] = useState(
    t("mobile.dashboard.greeting.afternoon")
  );
  ...
  console.log(
    "🚨 FORCE RELOAD: Dashboard component loaded at:",
      ...
      <div className="space-y-4 p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white p-4 rounded-lg shadow-sm">
            <div className="text-sm text-slate-500">
              {t("mobile.dashboard.stats.activeJobs")}
            </div>
            <div className="text-3xl font-bold mt-1">
              {statsLoading
                ? "..."
                : stats?.lavoriAttivi || defaultStats.lavoriAttivi}
            </div>
          </div>

          <div className="bg-white p-4 rounded-lg shadow-sm">
            <div className="text-sm text-slate-500">
              {t("mobile.dashboard.stats.totalClients")}
            </div>
            <div className="text-3xl font-bold mt-1">
              {statsLoading
                ? "..."
                : stats?.clientiTotali || defaultStats.clientiTotali}
            </div>
          </div>

          <div className="bg-white p-4 rounded-lg shadow-sm">
            <div className="text-sm text-slate-500">
              {t("mobile.dashboard.stats.completedJobs")}
            </div>
            <div className="text-3xl font-bold mt-1">
              {statsLoading
                ? "..."
                : stats?.lavoriCompletati || defaultStats.lavoriCompletati}
            </div>
          </div>

          <div className="bg-white p-4 rounded-lg shadow-sm">
            <div className="text-sm text-slate-500">
              {t("mobile.dashboard.stats.monthlyIncome")}
            </div>
            <div className="text-3xl font-bold mt-1">
              €
              {statsLoading
                ? "..."
                : stats?.incassoMensile || defaultStats.incassoMensile}
              ,00
            </div>
          </div>
        </div>
        ...
        <div className="mt-6">
          <h3 className="text-xl font-medium mb-3">
            {t("mobile.dashboard.appointments.title")}
          </h3>

          <div className="space-y-3">
            {appointmentsLoading ? (
              <div className="text-center py-4 text-slate-500">
                {t("mobile.dashboard.appointments.loading")}
              </div>
            ) : (
              (appointments || defaultAppointments).map((appointment: any) => (
                <div
                  key={appointment.id}
                  className={`border-l-4 border-${appointment.color}-500 bg-white p-3 rounded-r-lg shadow-sm`}
                  style={{
                    borderLeftColor:
                ...
                >
                  ...
                  <div className="mt-2">
                    <span
                      className={`bg-${appointment.color}-100 text-${appointment.color}-700 text-xs font-medium px-2 py-1 rounded`}
                      style={{
                        backgroundColor:
                          appointment.color === "blue"
                            ? "#dbeafe"
        ...
        <div className="mt-6">
          <h3 className="text-xl font-medium mb-3">
            {t("mobile.dashboard.reports.title")}
          </h3>
          ...
          <div className="grid grid-cols-1 gap-3">
            <div className="bg-white p-4 rounded-lg shadow-sm border-l-4 border-green-500">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm text-slate-500">
                    {t("mobile.dashboard.reports.efficiency")}
                  </div>
                  <div className="text-2xl font-bold text-green-600">
                    {statsLoading
                      ...
                      ? Math.round(
                          (stats.lavoriCompletati / stats.totalJobs) * 100
                        )
                      : 0}
                    %
                  </div>
                ...
                <div>
                  ...
                  <div className="text-2xl font-bold text-blue-600">
                    ...
                    {statsLoading
                      ...
                      ? Math.round(stats.incassoMensile / stats.totalJobs)
                      : 0}
                  </div>
                </div>
