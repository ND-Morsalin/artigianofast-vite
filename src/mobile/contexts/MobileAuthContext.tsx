...
export const MobileAuthContext = createContext<MobileAuthContextType | null>(
  null
);
...
export const MobileAuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      const error =
        err instanceof Error ? err : new Error("Errore durante il login");
      setError(error);
      throw error;
    } finally {
      setIsLoading(false);
  ...
  const logout = async () => {
    try {
      ...
      const error =
        err instanceof Error ? err : new Error("Errore durante il logout");
      setError(error);
      throw error;
    } finally {
      setIsLoading(false);
  ...
  const activate = async (code: string, password: string, fullName: string) => {
    try {
      ...
      const error =
        err instanceof Error
          ? err
          : new Error("Errore durante l'attivazione dell'account");
      setError(error);
      throw error;
    } finally {
      setIsLoading(false);
  ...
  const updateUser = async (userData: Partial<MobileUser>) => {
    try {
      ...
      const error =
        err instanceof Error
          ? err
          : new Error("Errore durante l'aggiornamento dei dati");
      setError(error);
      throw error;
    } finally {
      setIsLoading(false);
  ...
  const changePassword = async (
    ...
    try {
      ...
      const error =
        err instanceof Error
          ? err
          : new Error("Errore durante il cambio password");
      setError(error);
      throw error;
    } finally {
      setIsLoading(false);
...
export const useMobileAuth = () => {
  ...
  if (!context) {
    throw new Error(
      "useMobileAuth deve essere utilizzato all'interno di un MobileAuthProvider"
    );
  }
  return context;
};
