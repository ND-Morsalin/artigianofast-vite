...
export function SubscriptionPlansGrid({
  ...
  const parsePlanFeatures = (
    ...
    try {
      ...
      console.error(
        `Errore nel parsing delle caratteristiche del piano ${plan.id}:`,
        e
      );
      return {};
    }
  ...
  const getPlanFeaturesList = (plan: SubscriptionPlan): string[] => {
    ...
    if (maxJobs !== undefined) {
      featuresList.push(
        maxJobs === -1 ? "Lavori illimitati" : `Massimo ${maxJobs} lavori`
      );
    }

    // Limite collaboratori
    const maxCollaborators =
      features.limits?.maxCollaborators || features.max_collaborators;
  ...
  const getPopularPlanIndex = (
    ...
    if (plans.length >= 3) {
      ...
      const paidPlans = plans.filter(
        (plan) => !plan.isFree && plan.monthlyPrice > 0
      );
      if (paidPlans.length === 0) return -1;
      if (paidPlans.length === 1)
        return plans.findIndex((p) => p.id === paidPlans[0].id);

      // Se ci sono almeno 2 piani a pagamento, il piano popolare è quello di mezzo
      const middleIndex =
        Math.floor(paidPlans.length / 2) - (paidPlans.length % 2 === 0 ? 1 : 0);
    ...
    return (
      <div className="text-center py-10">
        ...
        <p className="text-gray-500">
          Si è verificato un errore durante il caricamento dei piani. Riprova
        ...
        {plans
          ...
          .map((plan, i) => {
            ...
            return (
              <Card
                key={plan.id}
                className={`border shadow h-full relative flex flex-col ${
