...
export default function PlansPage() {
  ...
  const handleSubscribe = async () => {
    ...
    const isFreePlan =
      plan.isFree || parseFloat(plan.monthlyPrice || "0") === 0;
    ...
    if (isFreePlan) {
      ...
      if (!tempUser) {
        ...
        toast({
          ...
          description:
            "Per favore registrati o effettua il login per attivare il piano gratuito",
          variant: "default",
        });
      ...
      try {
        ...
        if (response.data) {
          ...
          throw new Error(
            errorData.message ||
              errorData.error ||
              "Errore durante l'attivazione del piano"
          );
        }
      } catch (error) {
        console.log(error);
        console.error(
          "Errore durante l'attivazione del piano gratuito:",
          error
        );
        toast({
          title: "Errore",
      ...
      if (!tempUser) {
        ...
        toast({
          title: "Autenticazione richiesta",
          description:
            "Per favore registrati o effettua il login per procedere con il pagamento",
          variant: "default",
        });
        // Store selected plan info for after registration
        sessionStorage.setItem(
          "pendingPlan",
          JSON.stringify({ planId: selectedPlan, billingType })
        );
        setLocation("/desktop/auth");
        return;
      }
