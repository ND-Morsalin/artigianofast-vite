      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            {t(
              "analytics.subtitle",
              "Monitor your system performance, user behavior, and revenue metrics"
            )}
          </p>
        </div>
        <div className="flex items-center space-x-2">
          ...
          >
            ...
            {showSensitiveData
              ? t("analytics.hideSensitive", "Hide Sensitive")
              : t("analytics.showSensitive", "Show Sensitive")}
          </Button>
      ...
      >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>
                  {t(
                    "analytics.overview.planDistribution",
                    "Plan Distribution"
                  )}
                </CardTitle>
                <CardDescription>
                  {t(
                    "analytics.overview.planDistributionDesc",
                    "How users are distributed across different plans"
                  )}
                </CardDescription>
              </CardHeader>
                  ...
                  <div key={index} className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm text-muted-foreground">
                        {plan.userCount}{" "}
                        {t("analytics.overview.users", "users")} (
                        {plan.percentage}%)
                      </span>
                    </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle>
                  {t("analytics.overview.featureUsage", "Feature Usage")}
                </CardTitle>
                <CardDescription>
                  {t(
                    "analytics.overview.featureUsageDesc",
        ...
        <TabsContent value="plans" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>
                {t(
                  "analytics.plans.detailedAnalysis",
                  "Detailed Plan Analysis"
                )}
              </CardTitle>
              <CardDescription>
                {t(
                  "analytics.plans.detailedAnalysisDesc",
                  "Comprehensive breakdown of plan performance"
                )}
              </CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <thead>
                    <tr className="border-b">
                      <th className="text-left p-2">
                        {t("analytics.plans.planName", "Plan Name")}
                      </th>
                      <th className="text-left p-2">
                        {t("analytics.plans.users", "Users")}
                      </th>
                      <th className="text-left p-2">
                        {t("analytics.plans.percentage", "Percentage")}
                      </th>
                      <th className="text-left p-2">
                        {t("analytics.plans.avgUsage", "Avg Usage")}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
        ...
        <TabsContent value="users" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>
                  {t("analytics.users.featureAdoption", "Feature Adoption")}
                </CardTitle>
                <CardDescription>
                  {t(
                    "analytics.users.featureAdoptionDesc",
                    "How quickly users adopt new features"
                  )}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {userBehavior.featureAdoptionRate
                  .slice(0, 5)
                  .map((feature, index) => (
                    <div key={index} className="space-y-2">
                      <div className="flex items-center justify-between">
            ...
            <Card>
              <CardHeader>
                <CardTitle>
                  {t("analytics.users.mostUsedFeatures", "Most Used Features")}
                </CardTitle>
                <CardDescription>
                  {t(
                    "analytics.users.mostUsedFeaturesDesc",
                    "Features with highest usage rates"
                  )}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {userBehavior.mostUsedFeatures
                  .slice(0, 5)
                  .map((feature, index) => (
                    <div
                      key={index}
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {feature.uniqueUsers}{" "}
                          {t("analytics.users.uniqueUsers", "unique users")}
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        <TabsContent value="revenue" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>
                {t("analytics.revenue.breakdown", "Revenue Breakdown")}
              </CardTitle>
              <CardDescription>
                {t(
                  "analytics.revenue.breakdownDesc",
                  "Detailed revenue analysis by plan"
                )}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      <th className="text-left p-2">
                        {t("analytics.revenue.planName", "Plan Name")}
                      </th>
                      <th className="text-left p-2">
                        {t("analytics.revenue.users", "Users")}
                      </th>
                      <th className="text-left p-2">
                        {t("analytics.revenue.monthly", "Monthly")}
                      </th>
                      <th className="text-left p-2">
                        {t("analytics.revenue.yearly", "Yearly")}
                      </th>
                      <th className="text-left p-2">
                        {t("analytics.revenue.total", "Total")}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {revenueMetrics.planRevenueBreakdown.map((plan, index) => (
                      <tr key={index} className="border-b">
                        ...
                        <td className="p-2">
                          $
                          {showSensitiveData
                            ? plan.yearlyRevenue.toFixed(2)
                            : "***"}
                        </td>
                        <td className="p-2">
                          $
                          {showSensitiveData
                            ? plan.totalRevenue.toFixed(2)
                            : "***"}
                        </td>
                      </tr>
                    ))}
              ...
              <div className="mt-6 p-4 bg-muted rounded-lg">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    <div className="text-sm text-muted-foreground">
                      {t(
                        "analytics.revenue.monthlyRecurring",
                        "Monthly Recurring"
                      )}
                    </div>
                    <div className="text-2xl font-bold">
        ...
        <TabsContent value="charts" className="space-y-6">
          <AdvancedCharts
            ...
            featureUsageData={featureAnalytics.monthlyFeatureUsage.map(
              (month) => ({
                ...
                collaboratorManagement:
                  month.features.collaboratorManagement || 0,
            ...
            systemPerformanceData={systemPerformance.dailyMetrics.map(
              (day) => ({
                name: day.date,
