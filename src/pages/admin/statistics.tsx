...
export default function StatisticsPage() {
  ...
  const expirationData = [
    {
      name: "In scadenza (30gg)",
      value: stats?.subscriptionsStats?.expiringSoon || 0,
    },
    {
      name: t("admin.plans.isActive"),
      value:
        (stats?.subscriptionsStats?.active || 0) -
        (stats?.subscriptionsStats?.expiringSoon || 0),
    },
  ];
  ...
  return (
    <div className="container mx-auto py-8">
          ...
          <Select value={activePeriod} onValueChange={setActivePeriod}>
            ...
            <SelectContent>
              <SelectItem value="week">
                {t("admin.statistics.weekly")}
              </SelectItem>
              <SelectItem value="month">
                {t("admin.statistics.monthly")}
              </SelectItem>
              <SelectItem value="quarter">
                {t("admin.statistics.quarterly")}
              </SelectItem>
              <SelectItem value="year">
                {t("admin.statistics.yearly")}
              </SelectItem>
              <SelectItem value="all">
                {t("admin.statistics.allTime")}
              </SelectItem>
            </SelectContent>
          </Select>
          <Button
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>{t("admin.statistics.detailedAnalysis")}</CardTitle>
          <CardDescription>
            {t("admin.statistics.completeStats")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
              <h3 className="text-sm text-slate-500 mb-1">
                {t("admin.statistics.totalSubscriptions")}
              </h3>
              <p className="text-2xl font-bold">
                {stats?.subscriptionsStats?.total || 0}
              </p>
            ...
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
              <h3 className="text-sm text-slate-500 mb-1">
                {t("admin.statistics.totalClients")}
              </h3>
              <p className="text-2xl font-bold">
                {stats?.clientsStats?.total || 0}
              </p>
              <p className="text-xs text-green-600 mt-2">
                +{stats?.clientsStats?.new || 0}{" "}
                {t("admin.statistics.lastPeriod")}
              </p>
            </div>

            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
              <h3 className="text-sm text-slate-500 mb-1">
                {t("admin.statistics.totalRevenue")}
              </h3>
              <p className="text-2xl font-bold">
                {formatCurrency(stats?.revenueStats?.total || 0)}
              </p>
              <p className="text-xs text-green-600 mt-2">
                {formatCurrency(stats?.revenueStats?.monthly || 0)}{" "}
                {t("admin.statistics.monthlyLabel")}
              </p>
            </div>

            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
              <h3 className="text-sm text-slate-500 mb-1">
                {t("admin.statistics.avgSessionTime")}
              </h3>
              <p className="text-2xl font-bold">
                {stats?.usageStats?.avgSessionDuration || 0} min
              </p>
              <p className="text-xs text-slate-600 mt-2">
                {stats?.usageStats?.avgSessionsPerUser || 0}{" "}
                {t("admin.statistics.sessionsPerUser")}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-4 mb-6">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium">
                {t("admin.statistics.sector")}
              </span>
              <Select value={selectedSector} onValueChange={setSelectedSector}>
                <SelectTrigger className="w-[180px]">
                  <SelectValue placeholder={t("admin.statistics.allSectors")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">
                    {t("admin.statistics.allSectors")}
                  </SelectItem>
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium">
                {t("admin.statistics.plan")}
              </span>
              <Select value={selectedPlan} onValueChange={setSelectedPlan}>
                <SelectTrigger className="w-[180px]">
                  <SelectValue placeholder={t("admin.statistics.allPlans")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">
                    {t("admin.statistics.allPlans")}
                  </SelectItem>
                  {plans.map((plan: any) => (
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <SelectTrigger className="w-[180px]">
                  <SelectValue placeholder={t("admin.statistics.allTypes")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">
                    {t("admin.statistics.allTypes")}
                  </SelectItem>
      ...
      <Tabs defaultValue="subscriptions" className="w-full">
        <TabsList className="w-full mb-6">
          ...
          <TabsTrigger value="clients">
            {t("admin.statistics.clients")}
          </TabsTrigger>
          <TabsTrigger value="revenue">
            {t("admin.statistics.revenue")}
          </TabsTrigger>
          <TabsTrigger value="usage">{t("admin.statistics.usage")}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="subscriptions">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <ChartCard
              title={t("admin.statistics.distributionByPlanType")}
              subtitle={t("admin.statistics.activeSubscriptionsBreakdown")}
            >
            ...
            <ChartCard
              title={t("admin.statistics.expiringVsActive")}
              subtitle={t("admin.statistics.expiringNext30Days")}
            >
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
          ...
          <ChartCard
            title={t("admin.statistics.subscriptionTrend")}
            subtitle={t("admin.statistics.activeSubscriptionsEvolution")}
          >
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
              ...
              >
                ...
                <Line
                  type="monotone"
                  dataKey="active"
                  name={t("admin.statistics.activeSubscriptions")}
                  stroke="#00C49F"
                  activeDot={{ r: 8 }}
                />
                <Line
                  type="monotone"
                  dataKey="expiring"
                  name={t("admin.statistics.expiring")}
                  stroke="#FFBB28"
                />
                <Line
                  type="monotone"
                  dataKey="new"
                  name={t("admin.statistics.new")}
                  stroke="#0088FE"
                />
              </LineChart>
            </ResponsiveContainer>
            ...
            >
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Legend />
                  <Bar
                    dataKey="accessFrequency"
                    name="Frequenza Accessi"
                    fill="#8884d8"
                  >
                    {clientsByPlanData.map((entry: any, index: number) => (
                      <Cell
