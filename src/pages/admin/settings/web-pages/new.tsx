...
export default function NewWebPage() {
  ...
  useEffect(() => {
    const titleSubscription = form.watch(
      (value: Partial<z.infer<typeof formSchema>>, { name }: any) => {
        if (name === "title") {
          const currentSlug = form.getValues("slug") ?? "";
          const currentTitle = form.getValues("title") ?? "";
          // Se lo slug è vuoto o corrisponde allo slug generato dal titolo precedente,
          // aggiorniamo lo slug automaticamente
          if (
            currentSlug === "" ||
            currentSlug === generateSlug(currentTitle.replace(/.$/, ""))
          ) {
            const newTitle = value.title ?? currentTitle;
      ...
      <div className="flex items-center mb-6">
        ...
        <div>
          ...
          <p className="text-gray-500">
            Crea una nuova pagina per il tuo sito web
          </p>
        </div>
      </div>
