...
const availablePages = [
  ...
  { id: "jobs", name: "Lavori", description: "Gestione dei lavori" },
  {
    id: "activities",
    name: "Attività",
    description: "Monitoraggio delle attività",
  },
  {
    id: "calendar",
    name: "Calendario",
    description: "Visualizzazione del calendario",
  },
  ...
  },
  {
    id: "collaborators",
    name: "Collaboratori",
    description: "Gestione dei collaboratori",
  },
...
const availableFields = {
  clients: [
    ...
    { id: "email", name: "Email", description: "Indirizzo email" },
    {
      ...
      description: "Residenziale, commerciale, industriale",
    },
  ...
  jobs: [
    ...
    },
    {
      id: "startDate",
      name: "Data Inizio",
      description: "Data di inizio lavoro",
    },
    ...
    {
      id: "materialsCost",
      name: "Costo Materiali",
      description: "Costo dei materiali",
    },
    {
      id: "location",
      name: "Posizione",
      description: "Indirizzo del cantiere",
    },
    { id: "duration", name: "Durata", description: "Durata stimata" },
    {
      id: "completedDate",
      name: "Data Completamento",
...
export default function ClientPlanOverridesPage() {
  ...
  const form = useForm<ClientPlanOverrideFormValues>({
    resolver: zodResolver(
      clientPlanOverrideSchema
    ) as unknown as Resolver<ClientPlanOverrideFormValues>,
    defaultValues: {
      userId: 0,
      planId: 0,
      features: {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: t("error_title"),
        description:
          error.response?.data?.error || t("error_override_creation"),
        variant: "destructive",
      });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: t("error_title"),
        description:
          error.response?.data?.error || t("error_override_deletion"),
        variant: "destructive",
      });
    },
  });
  ...
  const getOverrideStatus = (override: any) => {
    ...
    try {
      ...
      const modifiedFeatures = Object.keys(features).filter(
        (key) =>
          key !== "client_override" &&
          key !== "override_reason" &&
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold">
            {t("admin.clientPlanOverrides.title")}
          </h1>
        </div>
        <div className="flex items-center gap-4">
          <LanguageSelector />
            ...
            <TableBody>
              {planConfigurations?.map((override: any) => (
                <TableRow key={override.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={override.isActive ? "default" : "secondary"}
                    >
              ...
              {selectedPlanFeatures && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h3 className="text-lg font-semibold text-blue-900 mb-3">
                    {t("base_plan_features")} -{" "}
                    {getPlanName(form.getValues("planId"))}
                  </h3>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {availableFeatures.map((feature) => {
                        const isEnabled =
                          selectedPlanFeatures[feature.id] === true;
                        return (
                          <div
                            key={feature.id}
                    ...
                    <div className="mb-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {availablePages.map((page) => {
                          const accessLevel =
                            selectedPlanFeatures.page_access[page.id];
                          ...
                          const getAccessText = (level: string) => {
                            switch (level) {
                              ...
                              case "view":
                                return t("readOnly");
                              case "none":
                                return t("notAccessible");
                              default:
                                return t("not_included");
                            }
                          };
                          return (
                            <div
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <div
                                  className={`w-3 h-3 rounded-full ${getAccessColor(
                                    accessLevel
                                  )}`}
                                ></div>
                                <div>
                              ...
                              {Array.isArray(fields) &&
                                fields.map((fieldId: string) => {
                                  const field = availableFields[
                                    section as keyof typeof availableFields
                                  ]?.find((f) => f.id === fieldId);
                                  return field ? (
                                    <div
                                      key={fieldId}
                  ...
                  {selectedPlanFeatures.permissions && (
                    <div className="mb-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {Object.entries(selectedPlanFeatures.permissions).map(
                          ([permission, enabled]: [string, any]) => (
                            <div
                              key={permission}
                              className="flex items-center justify-between p-3 bg-white rounded border"
                            >
                  ...
                  {selectedPlanFeatures.limits && (
                    <div className="mb-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {Object.entries(selectedPlanFeatures.limits).map(
                          ([limit, value]: [string, any]) => (
                            <div
                              key={limit}
                              className="flex items-center justify-between p-3 bg-white rounded border"
                            >
                  ...
                  <div className="space-y-4">
                    {availablePages.map((page) => (
                      <FormField
                        key={page.id}
                        control={form.control}
                        name={`features.page_access.${page.id}` as any}
                        render={({ field }) => (
                          <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                            <div className="space-y-0.5">
                              <FormLabel className="text-base">
