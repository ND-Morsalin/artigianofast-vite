...
const AdminSettingsInvoicePage: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      draft: {
        label: t("admin.invoice.status.draft"),
        className: "bg-gray-100 text-gray-800",
      },
      sent: {
        label: t("admin.invoice.status.sent"),
        className: "bg-blue-100 text-blue-800",
      },
      paid: {
        label: t("admin.invoice.status.paid"),
        className: "bg-green-100 text-green-800",
      },
      overdue: {
        label: t("admin.invoice.status.overdue"),
        className: "bg-red-100 text-red-800",
      },
      cancelled: {
        label: t("admin.invoice.status.cancelled"),
        className: "bg-gray-100 text-gray-800",
      },
    };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {t("admin.invoice.stats.totalInvoices")}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{invoices.length}</div>
            <p className="text-xs text-muted-foreground">
              {t("admin.invoice.stats.totalInvoicesDescription")}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {t("admin.invoice.stats.paidInvoices")}
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {t("admin.invoice.stats.paidInvoicesDescription")}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {t("admin.invoice.stats.pendingInvoices")}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
            ...
            <p className="text-xs text-muted-foreground">
              {t("admin.invoice.stats.pendingInvoicesDescription")}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {t("admin.invoice.stats.totalRevenue")}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
            ...
            <p className="text-xs text-muted-foreground">
              {t("admin.invoice.stats.totalRevenueDescription")}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {t("admin.invoice.invoicesListDescription")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {invoices.length === 0 ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>
                    {t("admin.invoice.table.invoiceNumber")}
                  </TableHead>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("admin.invoice.createInvoiceDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="clientId">
                  {t("admin.invoice.form.client")}
                </Label>
                <Select>
                  <SelectTrigger>
                    <SelectValue
                      placeholder={t("admin.invoice.form.selectClient")}
                    />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div>
              <Label htmlFor="description">
                {t("admin.invoice.form.description")}
              </Label>
              <Textarea
                id="description"
                placeholder={t("admin.invoice.form.descriptionPlaceholder")}
              />
            </div>
