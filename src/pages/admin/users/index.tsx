...
export default function UsersPage() {
  ...
  const updatePlanMutation = useMutation({
    mutationFn: async ({ userId, data }: { userId: number; data: any }) => {
      ...
      const userSubscriptions = subscriptions.filter(
        (s: any) => s.userId === userId && s.status === "active"
      );
    ...
    if (editingAdmin) {
      ...
      if (!data.password) {
        ...
        updateMutation.mutate({
          id: editingAdmin.id,
          data: restData as AdminFormValues,
        });
  ...
  const handleDelete = (id: number) => {
    if (
      confirm(
        "Sei sicuro di voler eliminare questo amministratore? Questa azione non può essere annullata."
      )
    ) {
  ...
  const openPlanDialog = (user: any) => {
    ...
    const userSubscriptions = subscriptions.filter(
      (s: any) => s.userId === user.id && s.status === "active"
    );
    const currentSubscription =
      userSubscriptions.length > 0
        ? userSubscriptions.reduce((latest: any, current: any) =>
            new Date(current.createdAt) > new Date(latest.createdAt)
              ? current
              : latest
          )
    ...
    planForm.reset({
      ...
      billingFrequency: currentSubscription
                  ...
                  <TableBody>
                    {Array.isArray(administrators) &&
                      administrators.map((user: any) => {
                        ...
                        const userPlan =
                          userSubscription && Array.isArray(plans)
                            ? plans.find(
                                (p: any) => p.id === userSubscription.planId
                              )
                            : null;

                        // Determina lo stato dell'abbonamento
                        let subscriptionStatus = t(
          ...
          <Card>
            ...
            <CardContent>
              {isLoadingAdmins ? (
                ...
                <div className="flex flex-col items-center justify-center py-10">
                  <p className="text-gray-500 mb-4">
                    {t("admin.users.noAdministratorsAvailable")}
                  </p>
                  <Button
                    onClick={openCreateDialog}
                    variant="outline"
                          ...
                          <TableRow key={admin.id}>
                            ...
                            <TableCell>
                              {roles && Array.isArray(roles)
                                ? roles.find((r: any) => r.id === admin.roleId)
                                    ?.name || t("admin.users.noRole")
          ...
          <Card>
            ...
            <CardContent>
              {isLoadingClients ? (
                ...
                <div className="flex flex-col items-center justify-center py-10">
                  <p className="text-gray-500 mb-4">
                    {t("admin.users.noClientsAvailable")}
                  </p>
                  <Button
                    onClick={() => setLocation("/admin/clients/new")}
                    variant="outline"
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {editingAdmin
                        ? "Nuova Password (lasciare vuoto per mantenere l'attuale)"
                        : "Password"}
                    </FormLabel>
                    <FormControl>
                      <Input
      ...
      <Dialog open={isPlanDialogOpen} onOpenChange={setIsPlanDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Modifica il piano di abbonamento per{" "}
