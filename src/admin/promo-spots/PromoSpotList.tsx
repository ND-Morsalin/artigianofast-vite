...
interface PromoSpotListProps {
  // onEdit can be called with a spot or with undefined when creating a new spot
  onEdit: any;
}
...
export default function PromoSpotList({ onEdit }: PromoSpotListProps) {
  ...
  // const [selectedSpot, setSelectedSpot] = useState<PromotionalSpot | null>(
  //   null
  // );
  ...
  const deleteSpotMutation = useMutation({
    ...
    onError: (error) => {
      ...
      toast({
        ...
        description: `${t("admin.settings.errorDuringDeletion")} ${
          error.message
        }`,
        variant: "destructive",
      });
  ...
  const toggleStatusMutation = useMutation({
    ...
    onError: (error) => {
      ...
      toast({
        title: t("admin.settings.spotError"),
        description: `${t("admin.settings.errorDuringStatusUpdate")} ${
          error.message
        }`,
        variant: "destructive",
      });
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center p-8">
        {t("admin.settings.loadingPromotionalSpots")}
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-red-500 p-8">
        {t("admin.settings.spotError")}: {error.message}
      </div>
    );
  }
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">
          {t("admin.settings.promotionalSpotManagement")}
        </h1>
        <Button
          ...
          {spots && spots.length > 0 ? (
                    ...
                    <TableCell>
                      {spot.startDate || spot.endDate ? (
                        <div className="flex flex-col gap-1 text-xs">
                          {spot.startDate && (
                            <div>
                              {t("admin.settings.spotFrom")}:{" "}
                              {format(new Date(spot.startDate), "dd/MM/yyyy")}
                            </div>
                          )}
                          {spot.endDate && (
                            <div>
                              {t("admin.settings.spotTo")}:{" "}
                              {format(new Date(spot.endDate), "dd/MM/yyyy")}
                            </div>
                          )}
                        </div>
                      ) : (
                        <span className="text-gray-500 text-xs">
                          {t("admin.settings.spotNoDateSet")}
                        </span>
                      )}
                    </TableCell>
                    <TableCell>
                      <Badge
                        variant={
                          spot.status === "active" ? "destructive" : "secondary"
                        }
                      >
                        {spot.status === "active"
                          ? t("admin.settings.spotActive")
                          : t("admin.settings.spotInactive")}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      {spot.createdAt &&
                        format(new Date(spot.createdAt), "dd/MM/yyyy", {
                          locale: it,
                        })}
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button
                          ...
                          title={
                            spot.status === "active"
                              ? t("admin.settings.spotDeactivate")
                              : t("admin.settings.spotActivate")
                          }
                        >
                          {spot.status === "active" ? (
                            <EyeOff size={16} />
            ...
            <div className="text-center py-8">
              <p className="text-gray-500">
                {t("admin.settings.noPromotionalSpotsAvailable")}
              </p>
              <Button className="mt-4" onClick={() => onEdit(undefined)}>
                {t("admin.settings.createFirstSpot")}
              </Button>
            </div>
          )}
        </CardContent>
