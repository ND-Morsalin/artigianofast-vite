...
type SpotFormValues = {
  ...
  title: string;
  content: string;
  position: "top" | "bottom" | "left" | "popup" | string;
  textAnimationType: "fixed" | "scroll" | string;
  imageDisplayType: "single" | "slideshow" | string;
...
export default function PromoSpotForm({ spot, onCancel }: PromoSpotFormProps) {
  ...
  const defaultValues: Partial<SpotFormValues> = {
    ...
    images: Array.isArray((spot as any)?.images)
      ...
      : typeof (spot as any)?.images === "string"
      ? (spot as any).images
      : "",
    height: spot?.height || 200,
    width: spot?.width || 300,
    startDate: spot?.startDate
      ? format(new Date(spot.startDate), "yyyy-MM-dd")
      : "",
  ...
  const spotFormSchema = z.object({
    ...
    textAnimationType: z.string().optional(),
    imageDisplayType: z.string().optional(),
    ...
    displayInterval: z.number().optional(),
    timeRanges: z
      .array(
        z.object({
      ...
      .optional(),
    visiblePages: z.union([z.string(), z.array(z.string())]).optional(),
  });

  const form = useForm<SpotFormValues>({
    resolver: zodResolver(
      spotFormSchema
    ) as unknown as Resolver<SpotFormValues>,
    defaultValues,
  });
  ...
  const createSpotMutation = useMutation({
    mutationFn: async (data: SpotFormValues) => {
      ...
      // Normalizza data.images in un array di stringhe (gestisce sia string che string[])
      ...
      const processedData = {
        ...
        enableRedirect:
          data.enableRedirect === undefined ? false : data.enableRedirect,
        // Serializza timeRanges come JSON string
        timeRanges:
          data.timeRanges &&
          data.timeRanges.length > 0 &&
          data.timeRanges[0].startTime &&
          data.timeRanges[0].endTime
        ...
        visiblePages: Array.isArray(data.visiblePages)
          ? data.visiblePages.length === 0
            ? "all"
            : JSON.stringify(data.visiblePages)
  ...
  const onSubmit = (data: SpotFormValues) => {
    if (isEditing && spot && typeof spot.id === "number") {
      updateSpotMutation.mutate({ ...data, id: spot.id });
    } else if (isEditing) {
      // If we're in editing mode but there's no valid id, abort and notify the user
      toast({
        title: "Errore",
        description: "ID dello spot mancante: impossibile aggiornare lo spot.",
        variant: "destructive",
      });
    } else {
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        <CardTitle>
          {isEditing ? "Modifica Spot Promozionale" : "Nuovo Spot Promozionale"}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="Inserisci il titolo dello spot"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      ...
                      <FormDescription>
                        {form.watch("position") === "top" &&
                          "Dimensioni consigliate: Larghezza 100%, Altezza 60-100px"}
                        {form.watch("position") === "bottom" &&
                          "Dimensioni consigliate: Larghezza 100%, Altezza 60-100px"}
                        {form.watch("position") === "left" &&
                          "Dimensioni consigliate: Larghezza 30-40%, Altezza 300-500px"}
                        {form.watch("position") === "popup" &&
                          "Dimensioni consigliate: Larghezza 80-90%, Altezza 300-500px"}
                      </FormDescription>
                    </Select>
                    <FormMessage />
                  </FormItem>
                  ...
                  <FormItem>
                    <div className="mb-4">
                      <FormLabel className="text-base">
                        Seleziona le pagine in cui mostrare lo spot:
                      </FormLabel>
                      <FormDescription>
                        Scegli "Tutte le pagine" oppure seleziona pagine
                        specifiche.
                      </FormDescription>
                    </div>
                      ...
                      <FormItem>
                        ...
                        <div className="border rounded-md p-4 mt-4">
                          ...
                          <div className="flex items-center justify-center w-full">
                            <label
                              htmlFor="imageUpload"
                              className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer bg-white hover:bg-gray-100/50"
                            >
            ...
            <div className="border rounded-md p-4 mb-4 bg-gray-50/10">
              <h3 className="text-md font-medium mb-4">
                Periodo di validità dello spot
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField
                  control={form.control}
            ...
            <div className="border rounded-md p-4 mb-4 bg-gray-50/10">
              ...
              <div className="space-y-3">
                {form
                  .watch("timeRanges")
                  ?.map((_: any, index: Key | null | undefined) => (
                    <div
                      key={index}
                    ...
                    >
                      <FormField
                        control={form.control}
                        name={`timeRanges.${index}.startTime` as unknown as any}
                        render={({ field }) => (
                      ...
                      <FormField
                        control={form.control}
                        name={`timeRanges.${index}.endTime` as unknown as any}
                        render={({ field }) => (
                          <FormItem className="flex-1">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (timeRanges.length > 1) {
                            form.setValue(
                              "timeRanges",
                              timeRanges.filter((_: any, i: any) => i !== index)
                            );
                          }
                        }}
              ...
              <FormDescription className="mt-2">
                Puoi aggiungere più fasce orarie per mostrare lo spot in diversi
                momenti della giornata. Ad esempio: 9:00-13:00, 14:00-18:00,
                22:00-24:00.
              </FormDescription>
            </div>
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
                    <div className="space-y-0.5">
                      ...
                      <FormDescription>
                        Attiva/disattiva il reindirizzamento quando si clicca
                        sull'immagine
                      </FormDescription>
                    </div>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Questo URL sarà utilizzato solo se il reindirizzamento è
                      attivo
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Tempo tra una visualizzazione e l'altra (0 = sempre
                      visibile)
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
