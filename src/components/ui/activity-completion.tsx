...
const activityCompletionSchema = z.object({
  ...
  actualDuration: z.number().min(0, "La durata deve essere maggiore di 0"),
  ...
  const completeActivity = useMutation({
    ...
    onError: (error) => {
      ...
      toast({
        title: "Errore",
        description:
          "Si è verificato un errore durante il completamento dell'attività.",
        variant: "destructive",
      });
              ...
              <CardContent className="py-2">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    {formatDate(new Date(jobActivity.startDate))} alle{" "}
                    {formatTime(new Date(jobActivity.startDate))}
                  </div>
                  <div>
                    <span className="font-medium">Durata programmata:</span>{" "}
                    {jobActivity.duration}{" "}
                    {jobActivity.duration === 1 ? "ora" : "ore"}
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="flex justify-end space-x-2 pt-2">
                ...
                <Button type="submit" disabled={completeActivity.isPending}>
