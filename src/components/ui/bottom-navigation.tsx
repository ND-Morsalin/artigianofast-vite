...
export function BottomNavigation({
  ...
  const isMobileActive = (path: string) => {
    if (
      path === "/mobile" &&
      (location === "/mobile" || location === "/mobile/dashboard")
    )
      return true;
    if (path === "/mobile/report" && location.startsWith("/mobile/reports/"))
      return true;
    ...
    <nav className="fixed bottom-0 w-full bg-white border-t border-neutral-200 py-2 px-1 flex justify-around items-center z-10">
      ...
      >
        <div
          className={cn(
            "flex items-center justify-center rounded-full w-6 h-6",
            isMobileActive("/mobile/calendar")
              ? "bg-blue-600 text-white"
              : "bg-gray-100 text-gray-600"
          )}
        >
      ...
      <button
        className="flex flex-col items-center justify-center"
        onClick={() =>
          handleTabClick("jobRegistration", "/mobile/registration")
        }
      >
        <div
          className={cn(
            "flex items-center justify-center rounded-full w-6 h-6",
            isMobileActive("/mobile/registration")
              ? "bg-blue-600 text-white"
              : "bg-gray-100 text-gray-600"
          )}
        >
      ...
      >
        <div
          className={cn(
            "flex items-center justify-center rounded-full w-6 h-6",
            isMobileActive("/mobile/settings")
              ? "bg-blue-600 text-white"
              : "bg-gray-100 text-gray-600"
          )}
        >
