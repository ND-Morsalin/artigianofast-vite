...
type JobActivityRegistrationFormValues = z.infer<
  typeof jobActivityRegistrationSchema
>;

export function JobActivityRegistration({
  job,
  onCompleted,
}: JobActivityRegistrationProps) {
  ...
  const activities = useMemo(() => {
    ...
    return allActivities.filter((activity) => {
      ...
      if (
        Array.isArray(activity.jobTypeIds) &&
        activity.jobTypeIds.includes(selectedJobTypeId)
      ) {
        return true;
      }
      ...
      if (typeof activity.jobTypeIds === "string") {
        try {
          ...
          if (
            Array.isArray(jobTypeIds) &&
            jobTypeIds.includes(selectedJobTypeId)
          ) {
            return true;
          }
        } catch (e) {
          ...
          if (
            activity.jobTypeIds &&
            activity.jobTypeIds.includes(String(selectedJobTypeId))
          ) {
            return true;
          }
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validFiles.length !== newMediaFiles.length) {
      toast({
        title: "Attenzione",
        description:
          "Alcuni file non sono stati aggiunti perché non sono immagini o video.",
        variant: "destructive",
      });
    }
              ...
              <CardContent className="py-2">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    {formatDate(new Date(job.startDate))} alle{" "}
                    {formatTime(new Date(job.startDate))}
                  </div>
                  <div>
                ...
                {jobActivities
                  ...
                  .map((activity) => {
                    ...
                    return (
                      <div
                        key={activity.id}
                        className="flex justify-between items-center py-1 border-b border-neutral-200 last:border-0"
                      >
                        <div>
                          <span className="font-medium">
                            {activityData?.name || `Attività #${activity.id}`}
                          </span>
                          <span className="ml-2 text-sm text-neutral-600">
                            {formatDate(new Date(activity.startDate))}{" "}
                            {formatTime(new Date(activity.startDate))}
                          </span>
                        </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        >
                          <SelectTrigger>
                            <SelectValue
                              placeholder={
                                !selectedJobTypeId
                                  ? "Seleziona prima un tipo di lavoro"
                                  : "Seleziona attività"
                              }
                            />
                          </SelectTrigger>
                  ...
                  <FormItem>
                    ...
                    {saveAsDraft && (
                      <p className="text-xs text-neutral-500 mt-1">
                        In modalità bozza lo stato sarà automaticamente "In
                        corso"
                      </p>
                    )}
                    <FormMessage />
              ...
              <div className="flex justify-end space-x-2 pt-2">
                ...
                >
                  {registerActivity.isPending && saveAsDraft
                    ? "Salvando..."
                    : "Salva in draft"}
                </Button>
