      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="bg-white rounded-lg shadow p-4 h-24 animate-pulse"
            ></div>
          ))}
        </div>
        ...
        <div className="p-4">
          {todayJobs.length === 0 ? (
            ...
            todayJobs.map((job) => (
              <div
                key={job.id}
                className={`border-l-4 ${getJobBorderColor(job.type)} p-3 mb-3`}
              >
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-medium">{job.title}</h3>
                  <span
                    className={`text-sm ${getJobBgColor(
                      job.type
          ...
          <div className="p-4 flex items-start">
            ...
            <div>
              <p className="font-medium">
                Lavoro completato: Sostituzione caldaia
              </p>
              <p className="text-sm text-neutral-500">
                Ieri alle 16:45 - Cliente: Famiglia Neri
              </p>
            </div>
          </div>
          <div className="p-4 flex items-start">
            ...
            <div>
              <p className="font-medium">
                Nuovo cliente aggiunto: Luca Bianchi
              </p>
              <p className="text-sm text-neutral-500">
                2 giorni fa - Contatto: 345 123 4567
              </p>
            </div>
          </div>
          <div className="p-4 flex items-start">
            ...
            <div>
              <p className="font-medium">
                Appuntamento riprogrammato: Riparazione perdita
              </p>
              <p className="text-sm text-neutral-500">
                3 giorni fa - Cliente: Paolo Rossi
              </p>
            </div>
          </div>
        </div>
